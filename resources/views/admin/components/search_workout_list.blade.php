...
window.renderUnifiedWorkoutList = function(workouts, classes = []) {
    ...
    workouts.forEach(workout => {
        ...
        <div class="border border-gray-300 rounded-lg mb-4 bg-white shadow-sm overflow-hidden text-sm">
            ...
            <!-- Footer -->
            <div class="mt-2 text-sm border-t border-gray-200 px-3 py-3 flex items-center bg-gray-50 gap-4">
    ...
    return rows.map(row => {
        ...
        if (fmt === 'Straight Sets' && row.sets && row.sets.length > 0) {
            nestedSetsHtml = row.sets.map(set => {
                ...
                return `
                     <div class="flex items-center text-[15px] font-weight-500 pl-2 border-l-2 border-gray-200 my-1 gap-6">
                         <div class="flex-1 text-gray-500 text-[13px]">${exName} (Set)</div>
                         <div class="w-32 text-center text-gray-500">${setLoad} ${setUnit}</div>
                         <div class="w-24 text-right text-gray-500">${setReps} REPS</div>
                     </div>
                `;
            }).join('');
        }
...
function populateSearchCategories() {
    $.ajax({
        ...
        error: function(err) { console.error("Error fetching categories:", err); }
    });
}
